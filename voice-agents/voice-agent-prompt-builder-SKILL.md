...
14. **Ending -- three tiers with hard criteria**:
    - Positive: Only if all key conditions are met (discovery done, concrete value explained, price indication heard). Prospect commits to a follow-up with a concrete date.
    - Neutral: Reasonable call but one or more conditions missing. Prospect asks for info by email, no commitment.
...
7. **Was the ending calibrated correctly?** Did the prospect give a positive ending when conditions weren't met?
